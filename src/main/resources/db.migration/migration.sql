...
VALUES ('Какие операции входят в DDL?',
    ...
    ('После какого оператора нельзя будет восстановить данные?', 'TRUNCATE',
       ...
       ('Что такое согласованность (consistency)?',
        ...
        'Гарантирует, что транзакция будет полностью выполнена или потерпит неудачу, где транзакция представляет одну логическую операцию данных',
        'Во время выполнения транзакции параллельные транзакции не должны оказывать влияние на ее результат');
    ('1. Что будет, если мы кладем в HashMap ключ, у которого equals и hashCode определены некорректно?',
...
,
    ('2. Какой метод преобразует поток в список?' , 'collect()' , 'count()' , 'toArray()' , 'concat()' ),
    ('3. Чем можно заменить лямбда-выражение?' , 'Анонимный класс' , 'Стрим' , 'Локальный класс' , 'Функциональный интерфейс') ,
    ('4. Начальная емкость (default capacity) LinkedList?' , '0' , '11' , '10' , '16')
    ('5. При итерации по ArrayList что будет если удалить элемент?' , 'Ничего не будет' , 'Будет IllegalStateException' ,
    'Будет ClassCastException' , 'Будет ConcurrentModificationException')
    ('6. Выберите методы Map появившегося в Java 8?' , 'compute()' , 'allMatch()' , 'findFirst' , 'keySet()') ,
    ('7. Какой из этих функциональных интерфейсов реализует функцию, получающая на вход два экземпляра класса T и возвращающая на выходе экземпляр класса T?' ,
     'BinaryOperator' , 'Supplier' , 'Function' , 'Predicate') ,
    ('8. Почему Map не Collection?' , 'Совокупность пар ключ и значение' , 'Совокупность ключей' , 'Совокупность всех элементов' , 'Совокупность значений') ,
    ('9. Какие нововведения появились в Java 8?' , 'Дефолтные методы в интерфейсах' , 'Дженерики' , 'Абстракные методы' ,
    '-------------') ,
    ('10. Какой класс с самым полным объемом информации о временном контексте в календарной системе ISO-8601?' , 'ZonedDataTime' , 'LocalDataTime' ,
    'DataTime' , 'LocalDate') ,
    ('11. В какой версии java появились дженерики?' , '5' , '1' , '8' , '15')
