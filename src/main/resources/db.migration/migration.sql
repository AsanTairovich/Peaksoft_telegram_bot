...
('JavaCoreOne','Какие недостатки есть у Enum ?','К ним не применимы операторы >, <, >=, <=','Не можем проверить тип данных','Не можем определять методы для отдельных констант',1),
('JavaCoreOne','Могут ли у разных объектов быть одинаковые hashCode()?', 'Да','Нет','Честно,не знаю',1),
('JavaCoreOne','Выберите ссылочный типы данных?', 'String', 'char','boolean',1),
('JavaCoreOne','К abstract-классам можно применить модификатор final?','Нет','Да','Честно, не знаю',1),
('JavaCoreOne','Какие недостатки у ООП?Какие недостатки у ООП?', 'Снижает производительность - многие вещи технически реализованы иначе, поэтому они используют больше ресурсов','Сложность реализации большого функционала - т.к. на написание кода уходит больше времени','Трудно читается - нужно выискивать в коде функции и выяснять, за что они отвечают',1),
('JavaCoreOne','Внутри вложенного класса (Nested class) мы можем объявить перечисления (Enum)?','Нет','Да',1),
('JavaCoreOne','Какие преимущества у java?','Автоматическое управление памятью','многословный код','Развитые инструменты по созданию GUI приложений на чистой java',1),
...
('JavaCoreOne','Может ли быть абстрактный класс без абстрактных методов?','Да','Нет','Честно, не знаю',1),
('JavaCoreOne','Какое исключения проверяются компилятором на этапе компиляции?','Checked','Unchecked','Честно, не знаю',1),
('JavaCoreOne','Могут ли классы быть статическими?','Да','Нет','Честно, не знаю',1),
...
('JavaCoreOne','Локальные классы (Local Inner Class) могут объявлены как public?','Нет','Да',1),
('JavaCoreOne','Может ли один блок catch отлавливать сразу несколько исключений?','Да','Нет','Честно, не знаю',1),
('JavaCoreOne','Можно ли использовать строки в конструкции switch?','Да','Нет','Честно, не знаю',1),
('JavaCoreOne','Какой из перечисленных вариантов будет отличным кандидатом для ключа в HashMap?','String','StringBuffer','StringBuilder',1),
...
('JavaCoreOne','Какой память используется всем приложением?','Heap','Stack',1),
('JavaCoreOne','Какое связывание необходимо для реализации полиморфизма?','Позднее (late binding)','Ранее (early binding)',1),
