...
VALUES ('SQLQuestion','Какие операции входят в DDL?', 'ALTER', 'SELECT', 'DROP', 'DELETE',3),
    ...
    ('JavaCoreTwo','Можно ли создать стрим из коллекции?' , 'да' , 'нет' , 'честно, не знаю',2),
    ...
    ('JavaCoreTwo','Транзакция, достигающая своего завершения и фиксирующая свои результаты, сохраняет согласованность базы данных','Независимо от проблем изменения, сделанные успешно завершённой транзакцией, должны остаться сохраненными после возвращения системы в работу', 'Гарантирует, что транзакция будет полностью выполнена или потерпит неудачу, где транзакция представляет одну логическую операцию данных','Во время выполнения транзакции параллельные транзакции не должны оказывать влияние на ее результат',2) ,
    ...
    ('JavaCoreTwo','C помощью Enumeration можно добавлять/удалять элементы?' , 'да' , 'нет' , 'честно, не знаю',2) ,
    ...
    ('JavaCoreTwo','При реализации Queue нужно переопределять equals() и hashcode()?' , 'нет' , 'да', 'честно, не знаю',2) ,
    ('JavaCoreTwo','Начальная емкость (default capacity) ArrayList?' , '10' , '16' , '11' , '0',2) ,
    ('JavaCoreTwo','У функциональных интерфейсов могут быть другие не абстрактные методы?' , 'да' , 'нет' , 'честно, не знаю',2) ,
    ('JavaCoreTwo','Про интерфейс Set?' , 'Хранит уникальные элементы' , 'Хранит дублированные элементы' , 'Хранит элементы в порядке добавления',2) ,
    ...
    ('JavaCoreTwo','Как устроена HashMap?' , 'Внутри состоит из корзин и списка элементов, на которые ссылаются корзины' , 'Представляет отображение в виде дерева' , 'В его основе лежит LinkedHashMap',2) ,
